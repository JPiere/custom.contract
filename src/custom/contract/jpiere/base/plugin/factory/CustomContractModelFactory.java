 ...
 *
 *  Custom Contract Model Factory
 *
 *
 *  @author Hideaki Hagiwara(h.hagiwara@oss-erp.co.jp)
 *
 */
public class CustomContractModelFactory implements IModelFactory {

	private static CCache<String,Class<?>> s_classCache = new CCache<String,Class<?>>(null, "PO_Class", 20, false);
	private final static CLogger s_log = CLogger.getCLogger(CustomContractModelFactory.class);
	...
	public Class<?> getClass(String tableName)
	{
				...
				|| tableName.startsWith("JP_Estimation"))
		{
			...
			//	Search packages
			StringBuffer name = new StringBuffer("custom.contract.jpiere.base.plugin.org.adempiere.model").append(".M").append(className);
			...
			//	Adempiere Extension
			clazz = getPOclass("custom.contract.jpiere.base.plugin.org.adempiere.model.X_" + tableName, tableName);
			if (clazz != null)
			{
				s_classCache.put(tableName, clazz);
				return clazz;
