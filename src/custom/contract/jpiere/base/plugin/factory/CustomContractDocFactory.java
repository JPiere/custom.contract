 ...
 *
 */
public class CustomContractDocFactory implements IDocFactory {

	private final static CLogger s_log = CLogger.getCLogger(CustomContractDocFactory.class);
	...
	public Doc getDocument(MAcctSchema as, int AD_Table_ID, int Record_ID,
		...
		Doc doc = null;
		if(	 AD_Table_ID==MRecognition.Table_ID //1000188
	...
	public Doc getDocument(MAcctSchema as, int AD_Table_ID, ResultSet rs, String trxName) {
		...
		if(AD_Table_ID == MRecognition.Table_ID){
			className = "custom.contract.jpiere.base.plugin.org.compiere.acct.Doc_JPRecognition";
		}else if(AD_Table_ID == MEstimation.Table_ID){
			className = "custom.contract.jpiere.base.plugin.org.compiere.acct.Doc_JPEstimation";
		}else if(AD_Table_ID == MContract.Table_ID){
			className = "custom.contract.jpiere.base.plugin.org.compiere.acct.Doc_JPContract";
		}else if(AD_Table_ID == MContractContent.Table_ID){
			className = "custom.contract.jpiere.base.plugin.org.compiere.acct.Doc_JPContractContent";
		}else if(AD_Table_ID == MContractProcSchedule.Table_ID){
			className = "custom.contract.jpiere.base.plugin.org.compiere.acct.Doc_JPContractProcSchedule";
		}else {
			return null;
		}
